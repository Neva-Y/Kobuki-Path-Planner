...
void KobukiNavigationStatechart(
	...
	bool key_pause = ((1 << 16) & KeyState) ? true : false;

	KeyState = GetAsyncKeyState(VK_RIGHT);
	bool key_right = ((1 << 16) & KeyState) ? true : false;

	KeyState = GetAsyncKeyState(VK_LEFT);
	bool key_left = ((1 << 16) & KeyState) ? true : false;

	KeyState = GetAsyncKeyState(VK_UP);
	bool key_up = ((1 << 16) & KeyState) ? true : false;

	KeyState = GetAsyncKeyState(VK_DOWN);
	bool key_down = ((1 << 16) & KeyState) ? true : false;
